...
namespace sleepy
{
	...
	void Registers::SetFlag(BYTE flagMask)
	{
		F |= flagMask;
	}
	...
	void Registers::ResetFlag(BYTE flagMask)
	{
		F &= (flagMask ^ 0xFF);
	}
	...
	void Registers::InvertFlag(BYTE flagMask)
	{
		F ^= flagMask;
	}
	...
	bool Registers::ReadFlag(BYTE flagMask)
	{
		return (flagMask | F) == F;
	}
